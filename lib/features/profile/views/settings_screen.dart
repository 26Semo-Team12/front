...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void _showDeleteAccountDialog(ProfileViewModel viewModel) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('회원 탈퇴', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
            ...
            return ListView(
              children: [
                ...
                SwitchListTile(
                  ...
                  activeTrackColor: const Color(0xFFD6706D).withValues(alpha: 0.4),
                  value: themeVm.isDark,
                  onChanged: themeVm.toggle,
                ),
                SwitchListTile(
                  secondary: const Icon(Icons.shuffle),
                  title: const Text('랜덤 매칭 모드', style: const TextStyle(fontSize: 16)),
                  subtitle: Text(
                    (viewModel.currentUser?.isRandomModeEnabled ?? false) ? '일반 매칭 대신 랜덤 매칭을 사용합니다' : '관심사 기반 매칭을 사용합니다',
                    style: TextStyle(fontSize: 13, color: Colors.grey.shade500),
                  ),
                  activeThumbColor: const Color(0xFFD6706D),
                  activeTrackColor: const Color(0xFFD6706D).withValues(alpha: 0.4),
                ...
                SwitchListTile(
                  ...
                  title: const Text('위치 액세스 권한', style: TextStyle(fontSize: 16)),
                  subtitle: Text(
                    _locationConsent ? '위치 기반 서비스 이용 중' : '위치 정보 제공 거부됨',
                    style: TextStyle(fontSize: 13, color: Colors.grey.shade500),
                  ),
                  activeThumbColor: const Color(0xFFD6706D),
                  activeTrackColor: const Color(0xFFD6706D).withValues(alpha: 0.4),
